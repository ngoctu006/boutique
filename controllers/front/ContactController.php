	...
	*/
	public function postProcess()
	{
		if (Tools::isSubmit('submitMessage'))
		{
			...
			$tel = Tools::getValue('telephone'); // Html entities is not usefull, iscleanHtml check there is no bad html tags.
			
                        if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from))
				$this->errors[] = Tools::displayError('Invalid email address.');
			elseif (!$message)
				$this->errors[] = Tools::displayError('The message cannot be blank.');
                        elseif (!$nom = trim(Tools::getValue('nom')) || !$prenom = trim(Tools::getValue('prenom')))
				$this->errors[] = Tools::displayError('The nom or prenom cannot be blank.');
			elseif (!Validate::isCleanHtml($message))
				$this->errors[] = Tools::displayError('Invalid message');
			elseif (!Validate::isInt($tel))
                                	...
                                	if (!count($this->errors))
                                        {
                                            $var_list = array(
                                                                            '{fullname}' => $prenom.' '.$nom,
                                                                            '{telephone}' => $tel,
                                                                            '{votre_message}' => Tools::getValue('votre-message'),
	...
	*/
	public function initContent()
	{
		...
		$this->context->smarty->assign(array(
			...
			'telephone' => html_entity_decode($phone),
			'votre_message' => html_entity_decode($votre_message)
		));
