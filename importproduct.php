...
//	$product->price = (float)number_format($price / (1 + (float)$line[3] / 100), 6, '.', '');
        ...
        if($line[7]){
          ...
          if (count($listcategory) > 1) {
                ...
                if (Category::categoryExists((int) $listcategory[0])) {
                    ...
                    $product->id_category[] = (int) $listcategory[0];
                }
            }
        }else{
	...
	if (isset($datas['id_product']) && $datas['id_product']){
        ...
        StockAvailable::setQuantity((int)$product->id, 0, (int)$line[6], 1);
        /**
         * 
         * 
         * **/
        // add prix shop 2
        $product1 = new Product($product->id);
        $shopOthers = 3 ; // set id shop hard code
        $price2 = str_replace(',', '.', $line[3]);
        $price2 = (float)number_format($price2, 6, '.', '');
        $product1->price = $price2;
        $product1->active = (int)$line[8];
        $product1->id_tax_rules_group = 55;
        $product1->link_rewrite[$id_lang] = Tools::link_rewrite($product1->name[$id_lang]);
        $product1->id_shop_list = array($shopOthers);
        $product1->update();
        StockAvailable::setQuantity((int)$product1->id, 0, (int)$line[6], $shopOthers);
